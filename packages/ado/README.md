...
| `listPullRequestComments` | yes | Comment threads, most recently active first, with status, file and line (`null` off a file or line); system notices left out. `status: active` narrows it and `top` (default 50) caps it; `total` says how many matched |
