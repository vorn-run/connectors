...
import { adoConnector } from './index'
...
describe('serveIfEntryPoint', () => {
  it('starts nothing when the module was merely imported', () => {
    const serve = vi.fn()
    expect(serveIfEntryPoint(adoConnector, HERE, serve)).toBe(false)
  ...
  it('serves the connector when the module is the entry point', () => {
    ...
    expect(serveIfEntryPoint(adoConnector, self, serve)).toBe(true)
    expect(serve).toHaveBeenCalledWith(adoConnector)
  })
})

describe('the packaged connector', () => {
  it('reports the package version, so a stale install is visible in the handshake', () => {
    expect(adoConnector.id).toBe('ado')
    expect(adoConnector.version).toMatch(/^\d+\.\d+\.\d+/)
  })
})
