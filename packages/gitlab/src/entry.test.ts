...
import { gitlabConnector } from './index'
...
describe('serveIfEntryPoint', () => {
  it('starts nothing when the module was merely imported', () => {
    ...
    const serve = vi.fn()
    expect(serveIfEntryPoint(gitlabConnector, HERE, serve)).toBe(false)
  ...
  it('serves the connector when the module is the entry point', () => {
    ...
    expect(serveIfEntryPoint(gitlabConnector, self, serve)).toBe(true)
    expect(serve).toHaveBeenCalledWith(gitlabConnector)
  })
})

describe('the packaged connector', () => {
  it('reports the package version, so a stale install is visible in the handshake', () => {
    expect(gitlabConnector.id).toBe('gitlab')
    expect(gitlabConnector.version).toMatch(/^\d+\.\d+\.\d+/)
  })

  it('is exported under a name that says which connector it is', () => {
    // Guards a copy-paste from the package this one was scaffolded from: a
    // `linearConnector` export here would be wired into Vorn under the wrong
    // name and only noticed by whoever read the import.
    expect(gitlabConnector.name).toBe('GitLab')
  })
})
