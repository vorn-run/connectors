...
export function createKustoConnector(options: KustoConnectorOptions = {}) {
  const connectTo = options.connectImpl ?? ambientClient
  const clients = new Map<string, Promise<KustoClientLike>>()

  /**
   * One client per cluster, for the life of the process.
   *
   * The connector is a long-lived stdio process polling on a timer, and the
   * client holds the token cache; rebuilding it per poll would re-probe the
   * credential chain every time, which for the Azure CLI means spawning a
   * process.
   */
  function clientFor(clusterUrl: string): Promise<KustoClientLike> {
    let client = clients.get(clusterUrl)
    if (!client) {
      client = connectTo(clusterUrl)
      clients.set(clusterUrl, client)
    }
    return client
  }
  ...
  async function fetchRows(context: FetchContext): Promise<ConnectorItem[]> {
    ...
    const table = await runKustoQuery(await clientFor(clusterUrl), {
      database,
      query,
      // The limit is bound as a number: `take` wants a long, and a string
      // would be rejected by the query rather than by us.
      parameters: { [SINCE_PARAM]: since, [LIMIT_PARAM]: limit }
    })
    ...
    actions: [
      {
        ...
        async run(args, { config }) {
          const clusterUrl = normalizeClusterUrl(required(config, 'cluster', 'KUSTO_CLUSTER'))
          // The SDK rejects a missing required input before `run`, but not a
          // number or an object, and `String(123).trim()` would sail through.
          const query = typeof args.query === 'string' ? args.query.trim() : ''
          if (!query) throw new Error('query is required')
          const database =
            String(args.database ?? '').trim() || required(config, 'database', 'KUSTO_DATABASE')

          const table = await runKustoQuery(await clientFor(clusterUrl), { database, query })
          return {
