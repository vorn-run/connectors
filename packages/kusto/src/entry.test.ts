...
import { kustoConnector } from './index'
...
describe('serveIfEntryPoint', () => {
  it('starts nothing when the module was merely imported', () => {
    const serve = vi.fn()
    expect(serveIfEntryPoint(kustoConnector, HERE, serve)).toBe(false)
  ...
  it('serves the connector when the module is the entry point', () => {
    ...
    expect(serveIfEntryPoint(kustoConnector, self, serve)).toBe(true)
    expect(serve).toHaveBeenCalledWith(kustoConnector)
  })
})

describe('the packaged connector', () => {
  it('reports the package version, so a stale install is visible in the handshake', () => {
    expect(kustoConnector.id).toBe('kusto')
    expect(kustoConnector.version).toMatch(/^\d+\.\d+\.\d+/)
  })
})
