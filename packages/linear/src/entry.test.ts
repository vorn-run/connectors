...
import { linearConnector } from './index'
...
describe('serveIfEntryPoint', () => {
  it('starts nothing when the module was merely imported', () => {
    ...
    const serve = vi.fn()
    expect(serveIfEntryPoint(linearConnector, HERE, serve)).toBe(false)
  ...
  it('serves the connector when the module is the entry point', () => {
    ...
    expect(serveIfEntryPoint(linearConnector, self, serve)).toBe(true)
    expect(serve).toHaveBeenCalledWith(linearConnector)
  })
})
...
describe('the packaged connector', () => {
  it('reports the package version, so a stale install is visible in the handshake', () => {
    ...
    expect(linearConnector.version).toMatch(/^\d+\.\d+\.\d+/)
  })
  ...
  it('is exported under a name that says which connector it is', () => {
    ...
    // an `adoConnector` export here would be wired into Vorn under the wrong
