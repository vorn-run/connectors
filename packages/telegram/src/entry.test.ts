...
describe('serveIfEntryPoint', () => {
  it('starts nothing when the module was merely imported', () => {
    ...
    const serve = vi.fn()
    expect(serveIfEntryPoint(telegramConnector, HERE, serve)).toBe(false)
  ...
  it('serves the connector when the module is the entry point', () => {
    ...
    expect(serveIfEntryPoint(telegramConnector, self, serve)).toBe(true)
    expect(serve).toHaveBeenCalledWith(telegramConnector)
  })
})

describe('the packaged connector', () => {
  it('reports the package version, so a stale install is visible in the handshake', () => {
    expect(telegramConnector.id).toBe('telegram')
    expect(telegramConnector.version).toMatch(/^\d+\.\d+\.\d+/)
  })
